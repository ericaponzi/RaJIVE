...
n <- 50
pks <- c(100, 90, 50)
Y <- jive.data.sim(K =3, rankJ = 3, rankA = c(8, 6, 5), n = n,
                   pks = pks, dist.type = 1)

initial_signal_ranks <- c(8,6,5)
...
# generate some outliers in the simulated data
  ...
  Y_outliers1[cond, pstar1] <- Y_outliers1[cond, pstar1] + rnorm(0.1*n*pstar1, 10 , 1)  
  
  
  Y_outliers2 <- Y$sim_data[[2]]
  pstar2 <- sample(pks[2], pks[2]*0.05)
  Y_outliers2[cond, pstar2] <- Y_outliers2[cond, pstar2] + rnorm(0.1*n*pstar2, 10 , 1)  
  
  
  Y_outliers3<- Y$sim_data[[3]]
  pstar3 <- sample(pks[3], pks[3]*0.05)
  Y_outliers3[cond, pstar3] <- Y_outliers3[cond, pstar3] + rnorm(0.1*n*pstar3, 10 , 1)  
