...
n <- 100
pks <- c(200, 180, 100)
Y <- jive.data.sim(K =3, rankJ = 3, rankA = c(15, 12, 11), n = n,
                   pks = pks, dist.type = 1)

initial_signal_ranks <- c(15,12,11)
...
```{r}
ajive::decomposition_heatmaps(data.ajive, ajive_decomp)
```
...
```{r}
ajive::decomposition_heatmaps(Y_outliers, ajive_decomp_out)
```
...
```{r}
library(ggplot2)
decomposition_heatmaps_robustH(Y_outliers, ajive.results.robust.out)
```
